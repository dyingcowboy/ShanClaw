...
### Context Management
- **Model-aware context window** (`client.ResolveModelCapabilities` + `agent.ComputeEffectiveContextWindow`): auto-1M for Opus 4.6/4.7 and Sonnet 4.6, 200K for Sonnet 4.5/4 and Haiku 4.5, conservative 200K for unknown specific models (no tier fallback — guards against newly-released 200K-cap models silently widening). Per-agent / `req.ModelOverride` re-routes via `loop.RefreshContextWindow`. `agent.context_window_auto: false` honors the static `agent.context_window`. No `anthropic-beta` header is sent (retired 2026-04-30; auto on capable models since 2026-03-13).
