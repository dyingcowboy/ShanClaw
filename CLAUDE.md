...
- **WS handshake & capabilities**: on connect, daemon sends `User-Agent: shanclaw/<version> (<os>; <arch>)`, `X-ShanClaw-Daemon-Version: <semver>` (sourced from `cmd.Version` → `daemon.Version` at startup; `dev` for local builds), and — when `daemon.Capabilities` is non-empty — `X-ShanClaw-Capabilities: <token>,<token>` listing optional protocol features the daemon honors. Cloud reads these on the WS upgrade handler to gate features that would otherwise break older daemons (e.g. per-message `delivery_ack` tracking that requires acks Cloud can't get from a legacy daemon). Headers are additive: empty / absent header = legacy mode. Add a capability token in the same PR that lands the feature it advertises — advertising before implementing causes Cloud to activate flows the daemon cannot satisfy.
