...
### Context Management
- **Model-aware context window** (`client.ResolveModelCapabilities` + `agent.ComputeEffectiveContextWindow`): specific-model path returns the exact cap — 1M for Opus 4.6/4.7 and Sonnet 4.6 (prefix match), 200K for Sonnet 4.5/4 and Haiku 4.5, conservative 200K for unknown specific models (no tier fallback — guards against newly-released 200K-cap models silently widening). **Tier-only** resolution (`model_tier: medium / big / large` with no pinned `agent.model`) returns the priority-1 happy-path window: medium/big/large → 1M (sonnet-4-6 / opus-4-6 1M auto), small → 200K (haiku has no 1M variant). `"big"` and `"large"` are aliases. Failover off priority 1 (e.g. medium → sonnet-4-5 200K) is intentionally not pre-capped here — the reactive recovery layer handles the 400 with summary cap + emergency compaction + retry, and pre-capping every tier at 200K would self-impose 800K of headroom loss on every healthy turn (issue #109). Per-agent / `req.ModelOverride` re-routes via `loop.RefreshContextWindow`. **Ollama provider always disables auto** (`agent.EffectiveContextWindowAuto`) regardless of `agent.context_window_auto` — the Anthropic prefix table can't map local model names like `qwen3:8b`; honors the operator-configured static `agent.context_window` instead. No `anthropic-beta` header is sent (retired 2026-04-30; auto on capable models since 2026-03-13).
